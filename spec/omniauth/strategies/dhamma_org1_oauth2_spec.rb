...
require 'spec_helper'
require 'json'
require 'omniauth-dhamma_org1'
require 'stringio'

describe OmniAuth::Strategies::DhammaOrg1 do
  ...
  subject do
    OmniAuth::Strategies::DhammaOrg1.new(app, 'appid', 'secret', @options || {}).tap do |strategy|
  ...
  describe 'verify_hd' do
    ...
    it 'should raise error if options hd is set and wrong' do
      ...
      expect do
        subject.send(:verify_hd, access_token)
      end.to raise_error(OmniAuth::Strategies::DhammaOrg1::CallbackError)
    end
